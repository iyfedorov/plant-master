...
fn main() -> Result<()> {
    ...
    let mut id_inc: u16 = 100u16;

    let frame = CanFrame::new(StandardId::new(id_inc).unwrap(), &[1u8]).unwrap();
    match mcp2515.send_message(frame) {
        Ok(_) => display.print_next("Initial message sedn OK")?,
        Err(e) => display.print_next(format!("Error send {:?}", e).as_str())?,
    };
        ...
        if int_pin.is_low() {
            match mcp2515.read_message() {
                Ok(frame) => {
                    ...
                    log::info!("Master got {:?}, {:?}", _can_id, _data);
